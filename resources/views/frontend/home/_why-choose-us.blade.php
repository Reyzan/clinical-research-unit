<!-- Why Choose Us Start --> 
<section>
    <div class="container">
        <div class="pbmit-heading-subheading text-center animation-style3">
            <h4 class="pbmit-subtitle">Why Choose Us</h4>
            <h2 class="pbmit-title">Why Choose CRU RSCM?</h2>
            <div class="pbmit-heading-desc">
                A nationally mandated clinical research hub with integrated facilities and trusted expertise.
            </div>
        </div>
        <div class="row pt-xl-4">
            <div class="col-md-12 col-xl-4 why-choose-one-left-col">
                <div class="why-choose-one-left-area animation animated fadeIn">
                    <div class="pbmit-ihbox-style-8">
                        <div class="pbmit-ihbox-box">
                            <div class="pbmit-ihbox-heading-wrap">
                                <div class="pbmit-ihbox-icon">
                                    <div class="pbmit-ihbox-icon-wrapper pbmit-icon-type-icon">
                                        <svg id="line_icons" height="512" viewBox="0 0 74 74" width="512" xmlns="http://www.w3.org/2000/svg" data-name="line icons"><path d="m58.522 72h-46.436a1 1 0 0 1 -1-1v-6.879a1 1 0 0 1 1-1h46.436a1 1 0 0 1 1 1v6.879a1 1 0 0 1 -1 1zm-45.436-2h44.436v-4.879h-44.436z"></path><path d="m55.943 65.121h-18.08a1 1 0 0 1 -1-1v-9.632a1 1 0 0 1 1-1h9.737a1 1 0 0 1 .755.345l8.345 9.632a1 1 0 0 1 -.756 1.655zm-17.08-2h14.891l-6.609-7.632h-8.282z"></path><path d="m37.863 55.489h-18.382a1 1 0 0 1 -1-1v-3.956a1 1 0 0 1 1-1h18.382a1 1 0 0 1 1 1v3.956a1 1 0 0 1 -1 1zm-17.382-2h16.382v-1.956h-16.382z"></path><path d="m53.1 61.846a1 1 0 0 1 -.756-.346l-4.864-5.613a1 1 0 0 1 .234-1.509 12.242 12.242 0 0 0 5.616-8.03 15.428 15.428 0 0 0 -4.35-13.685 1 1 0 0 1 .191-1.593 3.407 3.407 0 0 0 1.713-2.95 3.289 3.289 0 0 0 -.021-.4 1 1 0 0 1 1.48-.993c8.008 4.456 11.9 12.67 10.167 21.438a21.4 21.4 0 0 1 -8.839 13.5.985.985 0 0 1 -.571.181zm-3.368-6.414 3.524 4.068a19.4 19.4 0 0 0 7.292-11.72 17.545 17.545 0 0 0 -7.8-18.465 5.427 5.427 0 0 1 -1.571 2.741 17.577 17.577 0 0 1 4.115 14.681 14.245 14.245 0 0 1 -5.558 8.695z"></path><path d="m39.548 44.083a1 1 0 0 1 -.44-.1l-12.665-6.214a1 1 0 0 1 -.457-1.339l11.633-23.714a1 1 0 0 1 1.338-.457l12.665 6.2a1 1 0 0 1 .458 1.341l-2.257 4.6a1 1 0 0 1 -1.24.5 3.271 3.271 0 0 0 -1.116-.194 3.41 3.41 0 0 0 -1.852 6.277 1 1 0 0 1 .356 1.282l-5.524 11.258a1 1 0 0 1 -.899.56zm-11.326-7.67 10.869 5.332 4.725-9.632a5.411 5.411 0 0 1 4.573-9.33l1.454-2.963-10.868-5.32z"></path><path d="m35.393 48.186a1 1 0 0 1 -.44-.1l-9.211-4.517a1 1 0 0 1 -.457-1.338l2.428-4.951a1 1 0 0 1 1.338-.458l9.21 4.518a1 1 0 0 1 .458 1.338l-2.428 4.951a1 1 0 0 1 -.574.506 1.034 1.034 0 0 1 -.324.051zm-7.872-5.976 7.415 3.637 1.547-3.155-7.415-3.636z"></path><path d="m48.359 18.982a1 1 0 0 1 -.44-.1l-7.019-3.447a1 1 0 0 1 -.458-1.338l3.445-7.023a1 1 0 0 1 1.338-.458l7.023 3.445a1 1 0 0 1 .452 1.339l-3.445 7.023a1 1 0 0 1 -.9.56zm-5.684-4.9 5.225 2.562 2.563-5.227-5.225-2.564z"></path><path d="m54.4 13.229a1 1 0 0 1 -.441-.1l-12.209-5.987a1 1 0 0 1 -.458-1.342l1.592-3.24a1 1 0 0 1 1.338-.46l12.2 5.985a1 1 0 0 1 .457 1.339l-1.587 3.246a1 1 0 0 1 -.574.506 1.026 1.026 0 0 1 -.318.053zm-10.871-7.443 10.408 5.105.71-1.448-10.408-5.1z"></path><path d="m47.466 33.533a5.415 5.415 0 1 1 5.414-5.415 5.421 5.421 0 0 1 -5.414 5.415zm0-8.83a3.415 3.415 0 1 0 3.414 3.415 3.419 3.419 0 0 0 -3.414-3.418z"></path><circle cx="47.466" cy="28.118" r="1.301"></circle><path d="m47.466 29.919a1.8 1.8 0 1 1 1.8-1.8 1.8 1.8 0 0 1 -1.8 1.8zm0-2.6a.8.8 0 1 0 .8.8.8.8 0 0 0 -.8-.802z"></path><circle cx="44.543" cy="59.562" r="1.554"></circle><path d="m44.542 61.616a2.054 2.054 0 1 1 2.058-2.054 2.057 2.057 0 0 1 -2.058 2.054zm0-3.108a1.054 1.054 0 1 0 1.058 1.054 1.055 1.055 0 0 0 -1.058-1.054z"></path></svg>
                                    </div>
                                </div>
                                <h2 class="pbmit-element-title">In-House CRU Laboratory</h2>
                            </div>
                            <div class="pbmit-ihbox-contents">
                                <div class="pbmit-heading-desc">All research work conducted in our dedicated CRU laboratory facilities.</div>
                            </div>
                        </div>
                    </div>
                    <div class="pbmit-ihbox-style-8">
                        <div class="pbmit-ihbox-box">
                            <div class="pbmit-ihbox-heading-wrap">
                                <div class="pbmit-ihbox-icon">
                                    <div class="pbmit-ihbox-icon-wrapper pbmit-icon-type-icon">
                                        <svg class="w-64 h-64" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="none"><path d="M9.98735 2.98926C11.2175 2.98926 12.2147 3.98648 12.2147 5.21662C12.2147 6.44676 11.2175 7.44398 9.98735 7.44398C8.75721 7.44398 7.75999 6.44676 7.75999 5.21662C7.75999 3.98648 8.75721 2.98926 9.98735 2.98926ZM9.98735 3.98926C9.3095 3.98926 8.75999 4.53877 8.75999 5.21662C8.75999 5.89447 9.3095 6.44398 9.98735 6.44398C10.6652 6.44398 11.2147 5.89447 11.2147 5.21662C11.2147 4.53877 10.6652 3.98926 9.98735 3.98926ZM3.01562 11.5001C3.01562 11.7763 2.79177 12.0001 2.51562 12.0001C2.23948 12.0001 2.01562 11.7763 2.01562 11.5001V9.01431C2.01562 8.50148 2.40167 8.0788 2.899 8.02104L3.01562 8.01431L5.77285 8.01372C5.63086 8.25897 5.53947 8.53718 5.51295 8.83406L5.50494 9.01431H3.01562V11.5001ZM17.49 12.0001C17.7661 12.0001 17.99 11.7763 17.99 11.5001V9.01431L17.9832 8.89769C17.9255 8.40035 17.5028 8.01431 16.99 8.01431L14.2056 8.01372C14.3525 8.26749 14.4453 8.55654 14.468 8.86505L14.4735 9.01431H16.99V11.5001C16.99 11.7763 17.2138 12.0001 17.49 12.0001ZM12.4735 8.01431C12.9863 8.01431 13.409 8.40035 13.4668 8.89769L13.4735 9.01431V11.5001C13.4735 11.7763 13.2496 12.0001 12.9735 12.0001C12.6974 12.0001 12.4735 11.7763 12.4735 11.5001V9.01431H7.50494V11.5001C7.50494 11.7763 7.28108 12.0001 7.00494 12.0001C6.7288 12.0001 6.50494 11.7763 6.50494 11.5001V9.01431C6.50494 8.50148 6.89098 8.0788 7.38832 8.02104L7.50494 8.01431H12.4735ZM16.8878 5.53069C16.8878 4.474 16.0312 3.61739 14.9745 3.61739C13.9178 3.61739 13.0612 4.474 13.0612 5.53069C13.0612 6.58737 13.9178 7.44398 14.9745 7.44398C16.0312 7.44398 16.8878 6.58737 16.8878 5.53069ZM14.0612 5.53069C14.0612 5.02629 14.4701 4.61739 14.9745 4.61739C15.4789 4.61739 15.8878 5.02629 15.8878 5.53069C15.8878 6.03509 15.4789 6.44398 14.9745 6.44398C14.4701 6.44398 14.0612 6.03509 14.0612 5.53069ZM5.01595 3.61739C6.07264 3.61739 6.92925 4.474 6.92925 5.53069C6.92925 6.58737 6.07264 7.44398 5.01595 7.44398C3.95927 7.44398 3.10266 6.58737 3.10266 5.53069C3.10266 4.474 3.95927 3.61739 5.01595 3.61739ZM5.01595 4.61739C4.51155 4.61739 4.10266 5.02629 4.10266 5.53069C4.10266 6.03509 4.51155 6.44398 5.01595 6.44398C5.52035 6.44398 5.92925 6.03509 5.92925 5.53069C5.92925 5.02629 5.52035 4.61739 5.01595 4.61739ZM2.5 13.0001C2.22386 13.0001 2 13.224 2 13.5001V14.5001C2 15.8808 3.11929 17.0001 4.5 17.0001H15.5C16.8807 17.0001 18 15.8808 18 14.5001V13.5001C18 13.224 17.7761 13.0001 17.5 13.0001H2.5ZM3 14.5001V14.0001H17V14.5001C17 15.3285 16.3284 16.0001 15.5 16.0001H4.5C3.67157 16.0001 3 15.3285 3 14.5001Z" fill="currentColor"></path></svg>
                                    </div>
                                </div>
                                <h2 class="pbmit-element-title">Biobank Database <br> & Recruitment</h2>
                            </div>
                            <div class="pbmit-ihbox-contents">
                                <div class="pbmit-heading-desc">Extensive biobank database with large-scale participant recruitment capacity.</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-12 col-xl-4 why-choose-one-center-col animation animated fadeIn">
                <div class="why-choose-one-center-area">
                    <div class="gap-20 top-inner-box">
                       
                    </div>
                    <div class="girl-circle-img-wrap">
                        <div class="girl-circle-img">
                            <img src="{{ asset('frontend/images/why-choose-us/why-choose-us-square.png') }}" alt="">
                        </div>
                        <div class="girl-circle-bg"></div>
                    </div>
                    <div class="half-circle-img">
                        <img src="{{ asset('frontend/images/demo-1/half-circle.png') }}" class="img-fluid" alt="">
                    </div>
                </div>
            </div>
            <div class="col-md-12 col-xl-4 why-choose-one-right-col">
                <div class="why-choose-one-right-area animation animated fadeIn">
                    <div class="pbmit-ihbox-style-8">
                        <div class="pbmit-ihbox-box">
                            <div class="pbmit-ihbox-heading-wrap">
                                <div class="pbmit-ihbox-icon">
                                    <div class="pbmit-ihbox-icon-wrapper pbmit-icon-type-icon">
                                        <svg class="w-64 h-64" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" fill="currentColor"><path d="M254.3,107.91,228.78,56.85a16,16,0,0,0-21.47-7.15L182.44,62.13,130.05,48.27a8.14,8.14,0,0,0-4.1,0L73.56,62.13,48.69,49.7a16,16,0,0,0-21.47,7.15L1.7,107.9a16,16,0,0,0,7.15,21.47l27,13.51,55.49,39.63a8.06,8.06,0,0,0,2.71,1.25l64,16a8,8,0,0,0,7.6-2.1l55.07-55.08,26.42-13.21a16,16,0,0,0,7.15-21.46Zm-54.89,33.37L165,113.72a8,8,0,0,0-10.68.61C136.51,132.27,116.66,130,104,122L147.24,80h31.81l27.21,54.41ZM41.53,64,62,74.22,36.43,125.27,16,115.06Zm116,119.13L99.42,168.61l-49.2-35.14,28-56L128,64.28l9.8,2.59-45,43.68-.08.09a16,16,0,0,0,2.72,24.81c20.56,13.13,45.37,11,64.91-5L188,152.66Zm62-57.87-25.52-51L214.47,64,240,115.06Zm-87.75,92.67a8,8,0,0,1-7.75,6.06,8.13,8.13,0,0,1-1.95-.24L80.41,213.33a7.89,7.89,0,0,1-2.71-1.25L51.35,193.26a8,8,0,0,1,9.3-13l25.11,17.94L126,208.24A8,8,0,0,1,131.82,217.94Z"></path></svg>
                                    </div>
                                </div>
                                <h2 class="pbmit-element-title">Partnership <br> & Agreements</h2>
                            </div>
                            <div class="pbmit-ihbox-contents">
                                <div class="pbmit-heading-desc">Internal and external RSCM collaborations with CTA and research BHP support.</div>
                            </div>
                        </div>
                    </div>
                    <div class="pbmit-ihbox-style-8">
                        <div class="pbmit-ihbox-box">
                            <div class="pbmit-ihbox-heading-wrap">
                                <div class="pbmit-ihbox-icon">
                                    <div class="pbmit-ihbox-icon-wrapper pbmit-icon-type-icon">
                                        <svg class="w-64 h-64" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" fill="none"><path d="M14.25 2C14.6642 2 15 2.33579 15 2.75V3H22.75C24.5449 3 26 4.45507 26 6.25V16.75C26 16.8881 25.8881 17 25.75 17C25.6119 17 25.5008 16.8879 25.4923 16.7501C25.4358 15.8338 25.0709 15.001 24.5 14.3542V6.25C24.5 5.2835 23.7165 4.5 22.75 4.5H5.25C4.2835 4.5 3.5 5.2835 3.5 6.25V18.75C3.5 19.7165 4.2835 20.5 5.25 20.5H16.3726C15.831 20.8565 15.41 21.3813 15.1839 22H10.584L7.30878 25.7402C7.0359 26.0518 6.56207 26.0832 6.25044 25.8104C5.93882 25.5375 5.90741 25.0636 6.18029 24.752L8.59015 22H5.25C3.45507 22 2 20.5449 2 18.75V6.25C2 4.45507 3.45507 3 5.25 3H13.5V2.75C13.5 2.33579 13.8358 2 14.25 2ZM6.74621 7.5C6.33193 7.5 5.99609 7.83579 5.99609 8.25C5.99609 8.66421 6.33193 9 6.74621 9H12.2471C12.6613 9 12.9972 8.66421 12.9972 8.25C12.9972 7.83579 12.6613 7.5 12.2471 7.5H6.74621ZM6.74621 11.5C6.33193 11.5 5.99609 11.8358 5.99609 12.25C5.99609 12.6642 6.33193 13 6.74621 13H17.2479C17.6621 13 17.998 12.6642 17.998 12.25C17.998 11.8358 17.6621 11.5 17.2479 11.5H6.74621ZM6.74621 15.5C6.33193 15.5 5.99609 15.8358 5.99609 16.25C5.99609 16.6642 6.33193 17 6.74621 17H15.2475C15.6618 17 15.9977 16.6642 15.9977 16.25C15.9977 15.8358 15.6618 15.5 15.2475 15.5H6.74621ZM24.5 17C24.5 18.6569 23.1569 20 21.5 20C19.8431 20 18.5 18.6569 18.5 17C18.5 15.3431 19.8431 14 21.5 14C23.1569 14 24.5 15.3431 24.5 17ZM27 23.25C27 25.1178 25.4286 27 21.5 27C17.5714 27 16 25.1249 16 23.25V23.0448C16 21.9151 16.9158 21 18.0455 21H24.9545C26.0842 21 27 21.9151 27 23.0448V23.25Z" fill="currentColor"></path></svg>
                                    </div>
                                </div>
                                <h2 class="pbmit-element-title">Monitoring <br> & Evaluation</h2>
                            </div>
                            <div class="pbmit-ihbox-contents">
                                <div class="pbmit-heading-desc">Routine monitoring and comprehensive evaluation throughout research processes.</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="pbmit-bottom-text text-center mt-md-5 mt-4">
            Ready to collaborate? <a href="{{ route('contact') }}">Contact us</a> to discuss your research needs.
        </div>
    </div>
</section>
<!-- Why Choose Us end --> 